...
const areEqual = require("../deepEqual");
...
describe("areEqual", function () {
  ...
  it("works with dates", () => {
    ...
    expect(areEqual(new Date(2009, 9, 25), new Date(2009, 9, 25))).toBe(true);
    // Date objects referencing different times are not equal
    expect(areEqual(new Date(2009, 9, 25), new Date(2009, 11, 13))).toBe(false);
    // Date objects and objects with a `getTime` method are not equal
    expect(
      areEqual(new Date(2009, 11, 13), {
        getTime() {
          return 12606876e5;
        },
      })
  ...
  it("works with empty arrays, array-like objects, and object literals", function () {
  ...
  it("works with arrays containing primitive and object values", function () {
    ...
    expect(
      areEqual([/Moe/g, new Date(2009, 9, 25)], [/Moe/g, new Date(2009, 9, 25)])
    ).toBe(true);
  });
  ...
  it("works with multidimentional arrays", () => {
    const a = [
      ...
      /Moe/,
      new Date(2009, 11, 13),
      ["running", "biking", String("programming")],
      { a: 47 },
    ];
    const b = [
      Number(47),
  ...
  it("works with simple objects", function () {
    ...
    expect(
      areEqual({ a: "Curly", b: 1, c: true }, { a: "Curly", b: 1, c: true })
    ).toBe(true);
    // Objects containing equivalent members are equal
    expect(
      areEqual({ b: new Date(2009, 11, 13) }, { b: new Date(2009, 11, 13) })
    ).toBe(true);
  ...
  it("works with deep equality checks", () => {
    ...
    const b = { array: [1, 2, 3, { a: "1", b: [1, 2, 5] }] };
